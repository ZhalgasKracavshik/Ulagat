        ...
        <div className="container mx-auto py-10 px-4 max-w-5xl space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        {friends.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {friends.map((friend: any) => (
                                    <Card key={friend.id} className="hover:shadow-md transition-all group border-slate-100 overflow-hidden">
                                        <CardContent className="p-4 flex items-center justify-between">
                                            <Link href={`/profile/${friend.id}`} className="flex items-center gap-3 flex-grow min-w-0">
                                                ...
                                                <div className="truncate">
                                                    <p className="font-bold text-slate-900 group-hover:text-primary transition-colors truncate">{friend.full_name}</p>
                            ...
                            <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed">
                                ...
                                <p className="text-xs text-slate-400 mt-1">Search for people or visit their profiles to connect.</p>
                            </div>
                        )}
                    </section>
                </div>
                ...
                <div className="space-y-8">
                        ...
                        {incomingRequests.length > 0 ? (
                            <div className="space-y-3">
                                {incomingRequests.map((req: any) => (
                                    <Card key={req.id} className="border-indigo-100 bg-indigo-50/10 shadow-sm overflow-hidden">
                                        <CardContent className="p-4 space-y-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-bold text-sm text-slate-900">{req.requester?.full_name}</p>
                    ...
                    <section className="space-y-4">
                        <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Outbound</h2>
                        <div className="space-y-2">
                            {sentRequests.map((req: any) => (
                                <Card key={req.id} className="border-slate-50 bg-slate-50/50 shadow-none">
                                    <CardContent className="p-3 flex items-center justify-between">
                                        <div className="flex items-center gap-2 truncate">
                                            ...
                                            <span className="text-sm font-medium text-slate-600 truncate">{req.addressee?.full_name}</span>
                                        </div>
                                        <Badge variant="outline" className="text-[9px] font-bold text-slate-400 bg-white shadow-none">SENT</Badge>
                                    </CardContent>
                                </Card>
                            ))}
                            {sentRequests.length === 0 && (
                                <p className="text-[10px] text-slate-300 italic text-center">No outbound requests.</p>
                            )}
                        </div>
                    </section>
                </div>
