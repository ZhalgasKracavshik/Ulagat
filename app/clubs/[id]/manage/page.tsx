            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                    <Link href={`/clubs/${club.id}`} className="inline-flex">
                        <Button variant="ghost" size="sm" className="gap-2 text-slate-500 -ml-3">
                        ...
                        {club.status === 'active' ? (
                            <form action={recordMeeting} className="space-y-5">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="date" className="text-sm font-semibold text-slate-700">Meeting Date</Label>
                                    <Input
                                        id="date"
                                        name="date"
                                        type="date"
                                        ...
                                        max={todayIso}
                                        className="h-11 border-slate-200 rounded-lg shadow-sm"
                                    />
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        className="min-h-[80px] border-slate-200 resize-none rounded-lg shadow-sm"
                                    />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-sm font-semibold text-slate-700">Attendees</Label>
                                    {members.length > 0 ? (
                                        <div className="space-y-1 max-h-64 overflow-y-auto rounded-lg border border-slate-100 p-2">
                                            {members.map((member) => (
                                                <label
                                                    key={member.user_id}
                                                ...
                                                >
                                                    ...
                                                    <Avatar className="w-7 h-7 border">
                                                        ...
                                                        <AvatarFallback className="bg-violet-50 text-violet-600 text-xs">
                    ...
                    <Card className="border-violet-100">
                        ...
                        <CardContent>
                            <form action={postClubAnnouncement} className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="body" className="text-sm font-semibold text-slate-700">Message</Label>
                                    <Textarea
                                        id="body"
                                        name="body"
                                        placeholder="Write the announcement..."
                                        required
                                        className="min-h-[100px] border-slate-200 resize-none rounded-lg shadow-sm"
                                    />
                                </div>
                                <Button type="submit" className="w-full bg-violet-600 hover:bg-violet-700 font-bold h-11 rounded-lg">
                                    Post Announcement
                        ...
                        <CardContent className="space-y-2">
                            {meetings.length > 0 ? (
                                meetings.map((meeting) => (
                                    <div key={meeting.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-100 bg-slate-50/50">
                                        <span className="font-bold text-slate-800 text-sm tabular-nums shrink-0">
                                            {format(new Date(meeting.date + 'T00:00:00'), 'MMM d, yyyy')}
                                        </span>
                                        <span className="flex-grow min-w-0 text-sm text-slate-600 truncate">
                                            {meeting.notes || 'Club meeting'}
                                        </span>
                                        <span className="flex items-center gap-1 text-xs font-bold text-slate-500 bg-white border border-slate-200 px-2 py-0.5 rounded-full shrink-0">
